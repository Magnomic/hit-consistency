...
#include "ballot_box.h"                    // BallotBox 
...
DEFINE_int32(raft_max_parallel_append_entries_rpc_num, 1024,
...
void Replicator::_send_empty_entries(bool is_heartbeat) {
    ...
    if (_fill_common_fields(
        ...
        CHECK(!is_heartbeat);
        // impossible to get here if snapshot is disabled
        return;
    }
    if (is_heartbeat) {
